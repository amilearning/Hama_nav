...
template<typename T>
void set_veltamp(T & out)
{

        out.linear.x = kf_output.x_.vel(0);
        out.linear.y = kf_output.x_.vel(1);
        out.linear.z = kf_output.x_.vel(2);                
        // out.angular.x = angvel_avr(0);
        // out.angular.y = angvel_avr(1);
        // out.angular.z = angvel_avr(2);
...
geometry_msgs::PoseWithCovarianceStamped odometryToPoseWithCovariance(const nav_msgs::Odometry& odom) {
    geometry_msgs::PoseWithCovarianceStamped poseCovStamped;
    poseCovStamped.header.stamp = odom.header.stamp;  // Copy the timestamp
    poseCovStamped.header.frame_id = odom.header.frame_id;  // Copy the frame ID
    poseCovStamped.pose.pose = odom.pose.pose;
    return poseCovStamped;
}

void publish_odometry(const ros::Publisher & pubOdomAftMapped, const ros::Publisher & pubPoseAftMapped)
{
    ...
    set_veltamp(odomAftMapped.twist.twist);
    //////////////////////////// Publish posestamp ////////////////////////////
    geometry_msgs::PoseWithCovarianceStamped poseCovStamped = odometryToPoseWithCovariance(odomAftMapped);
    pubPoseAftMapped.publish(poseCovStamped);
    ////////////////////////////////////////////////////////////////////////////////////
    ...
    br.sendTransform( tf::StampedTransform( transform, odomAftMapped.header.stamp, "map", "livox" ) );
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher pubOdomAftMapped = nh.advertise<nav_msgs::Odometry> 
            ("/Odometry", 1000);
    ros::Publisher pubPoseAftMapped = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>  
            ("/pose", 1000);
            ...
            {     
                ...
                for (k = 0; k < time_seq.size(); k++)
                {
                    ...
                    if (publish_odometry_without_downsample)
                    {
                        ...
                        publish_odometry(pubOdomAftMapped, pubPoseAftMapped);
                        if (runtime_pos_log)
                        {
