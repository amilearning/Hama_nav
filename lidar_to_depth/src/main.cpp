...
public:
    ...
    0, 0, 1, 0, 0);
        ...
        point_cloud_sub = nh_.subscribe<livox_ros_driver::CustomMsg>("/livox/lidar", 1, &LidarToDepthConverter::pointCloudCallback, this);
...
private:
    ...
    void pointCloudCallback(const livox_ros_driver::CustomMsg::ConstPtr &msg){
        ...
        if (is_cam_ready){
            ...
            for (int i = 0; i < msg->point_num; i++) {
                ...
                float x_in_cam = -msg->points[i].y;
                float y_in_cam = -msg->points[i].z;
                float z_in_cam = msg->points[i].x;
