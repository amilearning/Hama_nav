...
def get_Intermediate_Points_generic(
    P0, P3, Vhat0, Vhat3, speed, compliment=False, jerk_opt=False
):
    global ratio
...
def Curv(t, KX1, KX2, KX3, KY1, KY2, KY3):
    ...
    dK = ((del3Y * delX - del3X * delY) / denominator) - (
        3
        * (delX * del2Y - del2X * delY)
        * (delX * del2X + delY * del2Y)
        / second_denominator
    )
...
def get_CTN(P1, P2, P3, P4, t):
    KP1 = (
        9 * P2 + 3 * P4 - 3 * P1 - 9 * P3
    )  # see how much shorter everything is when we use vector notation?
    KP2 = 6 * P1 - 12 * P2 + 6 * P3
    KP3 = 3 * (P2 - P1)
    B1 = (
        t * t * KP1 + t * KP2 + KP3
    )  # first derivative (also tangent) . Up to this line is the get-tangent function
...
def s_k(X, Y, slope1, destX, destY, slope2):
    ...
    Curvature = np.max(
        np.fabs(
            get_Curvature(X, Y, int1[0], int1[1], int2[0], int2[1], destX, destY, t)
        )
    )
...
def get_bezier_track(X, Y, slope):
    ...
    for i in range(len(X) - 1):
        k = i * 10
        bx[k : k + 10], by[k : k + 10] = get_bezier(
            X[i], X[i + 1], Y[i], Y[i + 1], slope[i], slope[i + 1]
        )
    return bx, by