...
import cv2
import numpy as np
from mppi_ctrl.control.mppi_offroad import mppi
...
class Hound_HL_Control:
    def __init__(self, Config):
        ...
        self.odom_sub = rospy.Subscriber(
            "/base_Odometry", Odometry, self.odom_callback
        )
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            if (
                ...
                tmp_goal, terminate, self.current_wp_index = self.update_goal(
                    self.goal,
    ...
    def update_goal(
        ...
        if goal is None:
            ...
            closest_index = np.argmin(
                np.linalg.norm(target_WP[:, :2] - pos[:2], axis=1)
            )
            terminate = False
            # if d < lookahead:
    ...
    def send_ctrl(self, ctrl):
        ...
        if control_msg.drive.speed > 3.0:
            control_msg.drive.speed = 3.0
    ...
    def grid_map_callback(self, grid_map):
        ...
        self.temp_map_elev = np.float32(
            cv2.flip(
                np.reshape(
                    matrix.data,
                    (matrix.layout.dim[0].size, matrix.layout.dim[1].size),
                    order="F",
                ).T,
                -1,
            )
        ...
        if self.debug:
            tmp_elev = self.temp_map_elev - np.min(self.temp_map_elev)
            scaled_costmap = (tmp_elev * 255).astype(np.uint8)        
            cv2.imwrite("elevmap.png", scaled_costmap)
    ...
    def process_grid_map(self):
        self.map_elev = np.copy(np.nan_to_num(self.temp_map_elev))
    ...
    def generate_cost(self, size, res, cent, _poses, track_width):
        ...
        if self.debug:
            scaled_costmap = (costmap * 255).astype(np.uint8)
            cv2.imwrite("costmap.png", scaled_costmap)
    ...
    def local_path_callback(self, path):
        ...
        for i in range(len(path.poses)):
            ...
            self.local_path_poses[i, 1] = path.poses[i].pose.position.y
